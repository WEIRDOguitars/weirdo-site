  ...
  if (color !== "natural") {
    ...
    if (burstApplied) {
      ...
      if (vivid && tintableWoodArea) {
        ...
        context.globalAlpha = isMaple ? .23 : .27;
        context.fillStyle = "#050505";
        context.fillRect(0, 0, width, height);
      }
      context.globalCompositeOperation = "color";
      context.globalAlpha = vivid ? .99 : .94;
      context.fillStyle = color;
      context.fillRect(0, 0, width, height);
      context.globalCompositeOperation = "multiply";
      context.globalAlpha = isMaple ? .46 : vivid ? .58 : .42;
      context.fillRect(0, 0, width, height);
    }
  }
...
function paintSolidGlossIntoTexture(context, width, height, area, color) {
  const side = area === "sides";
  context.save();
  context.globalCompositeOperation = "screen";
...
function selectedTopTexture(wood, color, area = "top", finish = "Mat") {
  ...
  if (label.includes("klon")) return color === "natural" ? textureMap("mapleFlame", 1, 1, 0, 0, area === "top" ? .1 : 0) : tintedTextureMap("mapleFlame", color, area, finish, 1, 1);
...
function glossCoatMaterial(role) {
  ...
  const opacity = solidPaint
    ? role === "top" ? .86 : .68
    : role === "top" ? .48 : .34;
  const strength = solidPaint
    ? role === "top" ? 1.55 : 1.16
    : role === "top" ? 1.0 : .72;
...
function paintTextureForColor(color, finish = "Mat", area = "top") {
  ...
  paintSolidGlossIntoTexture(context, width, height, area, color);
  forceOpaqueCanvas(context, width, height);
...
function woodMaterial(wood, color, finish, area = "top") {
  ...
  if (wood === "Jednolity kolor") {
    ...
    const paintMap = paintTextureForColor(color, finish, area);
